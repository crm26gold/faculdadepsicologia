...
import { demoRequested, isGoogleOwner, validConfiguration } from '@/lib/config';
...
export async function GET(request: Request) {
  ...
  if (!origin || !validConfiguration(process.env)) return new NextResponse('Acesso ainda não configurado.', { status: 503 });
